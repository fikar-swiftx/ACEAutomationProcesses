...
{
    static class ChangeMdProcessor
    {

        private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);


        public static void ProcessChangeMd(ChangeMdScenario scenario)
        {
            Log.Debug(
                $"Process ChangeinMD Scenario {scenario.Agency}, {scenario.CurrentPrcCode}, {scenario.Id}, {scenario.Seq}, {scenario.FormId}");
            ...
            if (currentAgency != null)
            {
                ...
                if (agencyFolder != null)
                {
                    ...
                    if (nricFolder == null)
                    {
                        ...
                        DbHelper.UpdateScenarioStatus("ChangeMD", "Fail", scenario.FormId, scenario.Seq);
                    }
                    else
                    {
                        ...
                        if (nricFolderNode.Name.Equals(scenario.Id) && scenario.Id.Equals(existingId) && scenario.Agency.Equals(existingAgency))
                        {
                            ...
                            bool success = OtcsHelper.UpdateChangeInMdAttributes(nricFolderNode.ID, scenario);
                            if (success)
                            {
                                Log.Debug(
                                    $"NRIC Folder exist under {scenario.Agency} ({agencyFolder.Name}) and metadata matches and successfully updated.");
                                DbHelper.UpdateScenarioStatus("ChangeMD", "Success", scenario.FormId, scenario.Seq);
                            }
                            else
                            {
                                //update fail
